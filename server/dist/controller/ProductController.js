...
const getProductsByCategory = async (req, res) => {
    try {
        ...
        const category = await CategoryModel_1.CategoryModel.findById(categoryId);
        if (!category) {
            res.status(404).json({ message: "Category not found!" });
            return;
        }
        const products = await ProductModel_1.Product.find({ category: categoryId })
...
const createProduct = async (req, res) => {
    try {
        ...
        // Check if category exists
        const categoryExists = await CategoryModel_1.CategoryModel.findById(category);
        if (!categoryExists) {
            res.status(404).json({ message: "Category not found!" });
            return;
        }
        ...
        const populatedProduct = await ProductModel_1.Product.findById(newProduct._id)
            .populate('category', 'title slug');
        res.status(201).json({
...
const updateProduct = async (req, res) => {
    try {
        ...
        if (hasChanges) {
            ...
            const updatedProduct = await ProductModel_1.Product.findByIdAndUpdate(_id, updateData, { new: true, runValidators: true }).populate('category', 'title slug');
            res.status(200).json({
            ...
            const populatedProduct = await ProductModel_1.Product.findById(_id)
                .populate('category', 'title slug');
            res.status(200).json({
...
const deleteProduct = async (req, res) => {
    try {
        ...
        await ReviewModel_1.Review.deleteMany({ productId: _id });
        await ProductModel_1.Product.findByIdAndDelete(_id);
        res.status(200).json({
